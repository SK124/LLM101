...
from archs.gpt2 import GPTModel
...
from utils.tensor_check import assign
from utils.generate import generate_textv2
from preprocessing.tokenprocessing import text_to_tokens, tokens_to_text
...
if __name__ == "__main__":
    ...
    trainer = L.Trainer(limit_train_batches=10, max_epochs=1, accelerator="mps", devices = 1)
    ...
    text_prompt = "What's good about the world?"
    input_prompt = text_to_tokens(text_prompt, tokenizer).to("mps")
    text = generate_textv2(gpt_model.model, idx = input_prompt, max_length = 128, context_size = 1024)
    print("INPUT_PROMPT", text_prompt)
    print("OUTPUT", tokens_to_text(text, tokenizer))